...
- **`packages/web`**: Static landing portal. In the self-hosted stack it doubles as the **front door / minified orchestrator** — Caddy (`infra/Caddyfile`) is the single entrypoint that routes each service by path prefix.
- **`wiki/`**: Docusaurus community wiki (built with `baseUrl: /wiki/`), served behind the front door at `/wiki`.

### Front-door routing (self-hosted)

| Path      | Service                                  |
| --------- | ---------------------------------------- |
| `/`       | static portal (`packages/web`)           |
| `/wiki/*` | Docusaurus wiki container                |
